...
class YandexChatService {
    ...
    async connect(): Promise<void> {
        ...
        return new Promise((resolve, reject) => {
            try {
                this.socket = new WebSocket(`${WS_URL}?token=${token}`); // Clean handshake to /ws path without root slash interference
    ...
    async sendMessage(chatId: string, text: string, recipientId: string) {
